...
#     if accelerator.is_main_process:  # 只让 rank 0 进程加载
#         optimizer.load_state_dict(torch.load(optimizer_path))
#         if is_warmup:
#             scheduler.load_state_dict(torch.load(scheduler_path))
#     accelerator.wait_for_everyone()
#     optimizer = accelerator.broadcast(optimizer)
#     scheduler = accelerator.broadcast(scheduler)
...
for epoch in range(num_epochs + 1):
    ...
    if (epoch + 1) % train_config["save_freq"]:
        ...
        # NOTE: The following validation codes are commented because it sometimes causing blocking.

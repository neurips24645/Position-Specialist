DraftPath=neurips24645/llama3-8b-instruct-poss1
	...
	do
		...
		do
			CUDA_VISIBLE_DEVICES=0 python -m evaluation.gen_poss_answer_llama3chat \
				...
				--temperature 1 \
				--model-id llama3-8b-poss1-depth${DEPTH}-tt60-iter${iter} \
				--forward_num_total 7 \
...
do
    ...
    do
		for iter in 1 2 3
		do
            FileName="llama3-8b-poss1-depth${DEPTH}-tt60-iter${iter}-temperature-1.0.jsonl"

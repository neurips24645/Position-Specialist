	...
	do
		...
		do
			CUDA_VISIBLE_DEVICES=0 python -m evaluation.gen_ea_answer_llama3chat \
				...
				--temperature 0 \
				--model-id llama3-8b-eagle-base-depth${DEPTH}-tt60-iter${iter} \
				--bench-name ${DATASET} \
				--total-token 60 \
				--depth ${DEPTH}
...
do
    ...
    do
		for iter in 1 2 3
		do
            FileName="llama3-8b-eagle-base-depth${DEPTH}-tt60-iter${iter}-temperature-0.0.jsonl"
